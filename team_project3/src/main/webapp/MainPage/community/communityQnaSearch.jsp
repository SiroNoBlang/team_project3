...
<body class="animsition">
	...
	<!-- Community page -->
		...
		<form action="CommunityQnaSearch.ma">
			<div class="form-group row justify-content-center">
				<div class="w100" style="padding-right:10px">
					<select class="form-control form-control-sm" name="searchType" id="searchType">
						<option value="qna_title">제목</option>
						<option value="qna_content" <c:if test="${param.searchType eq 'qna_content'}"> selected="selected"</c:if>>본문</option>
					</select>
				</div>
            ...
            <ul class="pagination justify-content-center">
		         <c:choose>
					<c:when test="${pageNum > 1}">
		             	 <li class="page-item"><a class="page-link" href="CommunityQnaSearch.ma?page=${pageNum - 1}">이전</a></li>
					</c:when>
					<c:otherwise>
				...
				<c:forEach var="i" begin="${startPage }" end="${endPage }">
					<c:choose>
						...
						<c:otherwise>     
			              	<li class="page-item"><a href="CommunityQnaSearch.ma?page=${i }" class="page-link">${i }</a></li>
			           	</c:otherwise>
					</c:choose>
				</c:forEach> 
	      		  ...
	      		  <c:choose>
						<c:when test="${pageNum < maxPage}">    
			            	<li class="page-item"><a class="page-link" href="CommunityQnaSearch.ma?page=${pageNum + 1}">다음</a></li>
			       	 	</c:when>
						<c:otherwise>   
